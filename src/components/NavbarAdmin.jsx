...
import { X, Home, LayoutDashboard, ClipboardList, Users, LogOut, User, Package, PlusCircle, ChevronDown, Menu } from "lucide-react";
    ...
    <nav style={{ position: "sticky", top: 0, zIndex: 50, background: "#fff", borderBottom: "1px solid #e5e0d8" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "0 16px" : "0 48px", height: "64px", maxWidth: "100%", boxSizing: "border-box" }}>
                ...
                <div style={{ position: "relative" }} onMouseEnter={openDropdown} onMouseLeave={closeDropdown}>
                  ...
                  {dropdownOpen && (
                    <div style={{ position: "absolute", top: "calc(100% + 1px)", left: 0, background: "#fff", border: "1px solid #e5e0d8", borderTop: "2px solid #1d4b8a", minWidth: "200px", boxShadow: "0 8px 24px rgba(15,31,53,0.10)", zIndex: 100, padding: "8px 0" }}>
                      <DropdownItem icon={<Package size={15} />} onClick={() => { navigate("/products"); setDropdownOpen(false); }}>Ver Productos</DropdownItem>
                      <DropdownItem icon={<PlusCircle size={15} />} onClick={() => { navigate("/products/add"); setDropdownOpen(false); }}>Agregar Producto</DropdownItem>
                      <DropdownItem icon={<ClipboardList size={15} />} onClick={() => { navigate("/orders"); setDropdownOpen(false); }}>Órdenes</DropdownItem>
                    </div>
                  )}
                </div>
              </div>
        ...
        {isMobile ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <span style={{ background: "#eef2f8", color: "#1d4b8a", border: "1px solid #ccd9ea", fontSize: "10px", fontWeight: 700, letterSpacing: "1px", padding: "4px 10px", fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase" }}>
              ADMIN
            </span>
            <button onClick={() => navigate("/users")} style={iconButtonStyle} title="Usuarios"
              onMouseEnter={e => e.currentTarget.style.background = "#f5f7fa"}
              onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
      ...
      <div style={{ position: "fixed", top: 0, left: menuOpen ? 0 : "-280px", width: "280px", height: "100vh", background: "#fff", zIndex: 200, display: "flex", flexDirection: "column", boxShadow: "4px 0 32px rgba(15,31,53,0.18)", transition: "left 0.3s ease" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 0" }}>
          ...
          <DrawerLink to="/products" icon={<LayoutDashboard size={18} />} onClick={() => setMenuOpen(false)}>Mis Productos</DrawerLink>
          <DrawerLink to="/orders" icon={<ClipboardList size={18} />} onClick={() => setMenuOpen(false)}>Gestión de Órdenes</DrawerLink>
          <DrawerLink to="/users" icon={<Users size={18} />} onClick={() => setMenuOpen(false)}>Usuarios</DrawerLink>
        </div>
        ...
        <div style={{ padding: "16px 24px" }}>
          <button
            ...
            CERRAR SESIÓN ADMIN <LogOut size={16} />
          </button>
        </div>
      </div>
    </nav>
